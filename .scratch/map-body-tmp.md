...
- **UI:** OpenCode/Pi–like ADE TUI is the primary human surface (activity, tools, plan/todos, chat, status/cost, approvals). Headless/`--no-tui` is for CI/tests only. Gemini-CLI behaviors for expand/scroll/model/quota are inspiration only (see research ticket 16).
- **Providers:** direct API keys for hackathon (`GROQ_API_KEY` working today; next: `GEMINI_API_KEY` + `gemini-3.5-flash-lite` via [Add Google Gemini provider](https://github.com/rajofearth/CodeRush2.0_TeamKnull/issues/17); also OpenAI/Anthropic/OpenRouter/Cerebras); AI Gateway optional later.
...
- [Wire LSP tools and repository intake map](https://github.com/rajofearth/CodeRush2.0_TeamKnull/issues/12) — TS LSP diagnostics/defs/refs + intake map; `clai demo lsp` / `clai intake`.
- [Research Gemini CLI / Pi ADE patterns for CLAI Ink](https://github.com/rajofearth/CodeRush2.0_TeamKnull/issues/16) — steal expandable tools, `/model`, soft rate-limit, stick-scroll; asset `16-gemini-pi-ade-patterns.md`.
- [Add Google Gemini provider (flash-lite default)](https://github.com/rajofearth/CodeRush2.0_TeamKnull/issues/17) — Gemini is first-class via `GEMINI_API_KEY` or its Google alias, defaults to confirmed `gemini-3.5-flash-lite`, and passes the headless tiny-fixture run.
...
- Feature parity with OpenCode / Pi / Cursor / Gemini CLI as a clone; study them, ship an original harness.
- Global AST fragment graph / embedding code search as the exploration path.
- Forking OpenCode/Pi/Gemini CLI as the submission.
